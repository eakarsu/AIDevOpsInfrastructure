/**
 * Gap Feature: gap-no-change-management-workflow
 * No change-management workflow.
 *
 * POST /api/gap-no-change-management-workflow
 * Auth required. Generated as part of Gaps backend scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_GAP_NO_CHANGE_MANAGEMENT_WORKFLOW_KEY
...
const FEATURE_KEY = process.env.FEATURE_GAP_NO_CHANGE_MANAGEMENT_WORKFLOW_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: No change-management workflow..\nGoal: No change-management workflow.\nRespond with clear, actionable analysis. Prefer JSON when structured output is requested.`;
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDevOpsInfrastructure - gap-no-change-management-workflow',
    },
    body: JSON.stringify({
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_GAP_NO_CHANGE_MANAGEMENT_WORKFLOW_KEY');
    const ai = await callLLM({ feature: 'gap-no-change-management-workflow', goal: 'No change-management workflow.', input: payload });
    try {
      ...
      if (pool && pool.query) {
        await pool.query(
          ...
          ['gap-no-change-management-workflow', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[gap-no-change-management-workflow] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'gap-no-change-management-workflow',
      endpoint: '/api/gap-no-change-management-workflow',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[gap-no-change-management-workflow] error:', err.message);
