    ...
    <nav className="sidebar">
      <div className="sidebar-header">
        <Link to="/" style={{ textDecoration: 'none' }}>
          <div className="sidebar-logo">AI DevOps Infra</div>
          <div className="sidebar-subtitle">Infrastructure Intelligence</div>
        </Link>
      </div>
      ...
      <div className="sidebar-nav">
        ...
        {features.slice(0, 4).map(f => (
          <Link key={f.key} to={`/${f.key}`} className={`nav-item ${location.pathname === `/${f.key}` ? 'active' : ''}`}>
