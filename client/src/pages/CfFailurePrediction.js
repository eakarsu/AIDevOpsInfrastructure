...
/**
 * Gap/CFS Feature Page: Failure prediction
 * Slug: failure-prediction
 * Backend: /api/cf-failure-prediction
 */
export default function FailurePredictionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-failure-prediction', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Failure prediction</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Predict infrastructure/component failures before they occur; schedule maintenance.</p>
