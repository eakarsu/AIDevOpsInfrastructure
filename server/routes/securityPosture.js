/**
 * Custom Feature: security-posture
 * Security posture automation
 *
 * POST /api/ai/security-posture
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_SECURITY_POSTURE_KEY
...
const FEATURE_KEY = process.env.FEATURE_SECURITY_POSTURE_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Security posture automation.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDevOpsInfrastructure - security-posture',
    },
    body: JSON.stringify({
...
router.post('/security-posture', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_SECURITY_POSTURE_KEY');
    const ai = await callLLM({ feature: 'security-posture', goal: 'Security posture automation', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['security-posture', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[security-posture] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'security-posture',
      endpoint: '/api/ai/security-posture',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[security-posture] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/security-posture/health', (req, res) => {
  res.json({
    feature: 'security-posture',
    endpoint: '/api/ai/security-posture',
