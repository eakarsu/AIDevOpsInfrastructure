...
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Infrastructure anomaly detection.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDevOpsInfrastructure - anomaly-detect',
    },
    body: JSON.stringify({
...
router.post('/anomaly-detect', auth, async (req, res) => {
  try {
    ...
    const ai = await callLLM({ feature: 'anomaly-detect', goal: 'Infrastructure anomaly detection', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
